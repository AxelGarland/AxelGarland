...
export function PrototypeMockup({ prototype, title, className = "" }: PrototypeMockupProps) {
  ...
  return (
    <div
      className={`overflow-hidden rounded-lg border border-ink/10 bg-paper shadow-[0_20px_50px_-30px_rgba(0,0,0,0.6)] ${className}`}
    >
      <div className="flex items-center gap-3 border-b border-ink/10 bg-paper-raised px-4 py-3">
        <div className="flex shrink-0 gap-1.5" aria-hidden>
          <span className="h-2.5 w-2.5 rounded-full bg-[#ec6a5e]" />
          <span className="h-2.5 w-2.5 rounded-full bg-[#f4bf4f]" />
          <span className="h-2.5 w-2.5 rounded-full bg-[#61c454]" />
        </div>
        <div className="mx-auto flex min-w-0 items-center gap-2 rounded-full bg-paper px-4 py-1.5 text-xs text-ink-subtle">
      ...
      <div className="relative aspect-[2560/1800] w-full bg-paper-raised">
        <Image
          key={screen.id}
        ...
        {screen.hotspots?.map((h) => (
          <button
            ...
            className="absolute rounded-md transition hover:bg-white/15 hover:ring-2 hover:ring-white/60 focus-visible:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/60"
            style={{
              left: `${h.xPct}%`,
              top: `${h.yPct}%`,
              width: `${h.wPct}%`,
        ...
        {screen.backTo ? (
          <button
            ...
            className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full bg-paper/90 px-3 py-1.5 text-xs font-medium text-ink shadow-md backdrop-blur-sm transition hover:bg-paper"
          >
            &larr; Back
          </button>
        ) : null}
